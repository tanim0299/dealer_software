 ...
 <div class="container">
    <div class="page-inner">
        <div
            ...
            @include('backend.layouts.partials.breadcrumb',['page_title'=>'List Of Sub Unit'])
            <div class="ms-md-auto py-2 py-md-0">
                <a href="{{route('sub_unit.create')}}" class="btn btn-primary  btn-round"><i class="fa fa-plus"></i> Create Sub Unit</a>
            </div>
        </div>
        <div class="row">
            <div class="card">
                <div class="card-body">
                    <form method="get" action="{{ route('sub_unit.index') }}">
                            ...
                            <tbody>
                                @forelse($data as $value)
                                    <tr>
                                        ...
                                        <td>{{ $value->name }}</td>
                                        <td>{{ $value->unit_data }}</td>
                                        <td>{{ \App\Models\SubUnit::STATUS[$value->status] }}</td>
                                        <td>
                                            <a href="{{ route('sub_unit.edit', $value->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                            <form action="{{ route('sub_unit.destroy', $value->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
