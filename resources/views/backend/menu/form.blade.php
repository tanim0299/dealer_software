        ...
        <div class="col-md-4 col-lg-4 col-12">
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('sl') is-invalid @enderror" id="sl" name="sl"
                    placeholder="Enter Sl" value="{{ old('sl', $menu->sl) }}">
                @error('sl')
                ...
                <select class="form-select @error('section_id') is-invalid @enderror" id="section_id" name="section_id">
                    ...
                    @forelse($sections as $section)
                        <option value="{{ $section->id }}"
                            @if(old('section_id', $menu->menu_section_id) == $section->id) selected @endif>
                            {{ $section->name }}
                        </option>
                    @empty
        ...
        <div class="col-md-4 col-lg-4 col-12">
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                    placeholder="Enter Name" value="{{ old('name', $menu->name) }}">
                @error('name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-md-4 col-lg-4 col-12    ">
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('system_name') is-invalid @enderror" id="system_name" name="system_name"
                    placeholder="Enter Name" value="{{ old('system_name', $menu->system_name) }}">
                @error('system_name')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-md-4 col-lg-4 col-12">
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('route') is-invalid @enderror" id="route" name="route"
                    placeholder="Enter Route Name" value="{{ old('route', $menu->route) }}">
                @error('route')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-md-4 col-lg-4 col-12">
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug"
                    placeholder="Enter Slug ( Eg : index, create, edit)" value="{{ old('slug', $menu->slug) }}">
                @error('slug')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div class="col-md-4 col-lg-4 col-12">
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('icon') is-invalid @enderror" id="icon" name="icon"
                    placeholder="" value="{{ old('icon', $menu->icon) }}">
                @error('icon')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status">
                    @foreach(\App\Models\Menu::STATUS as $key => $value)
                        <option value="{{ $key }}" @if(old('status', $menu->status)==$key) selected @endif>{{ $value }}</option>
                    @endforeach
                </select>
                @error('status')
                <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('type') is-invalid @enderror" id="type" name="type">
                    ...
                    @foreach(\App\Models\Menu::TYPE as $key => $value)
                        <option value="{{ $key }}" @if(old('type', $menu->type)==$key) selected @endif>{{ $value }}</option>
                    @endforeach
                </select>
                @error('type')
                <div class="invalid-feedback">{{ $message }}</div>
