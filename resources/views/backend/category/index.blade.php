 ...
 <div class="container">
    <div class="page-inner">
        <div
            ...
            @include('backend.layouts.partials.breadcrumb',['page_title'=>'List Of Category'])
            <div class="ms-md-auto py-2 py-md-0">
                <a href="{{route('category.create')}}" class="btn btn-primary  btn-round"><i class="fa fa-plus"></i> Create Category</a>
            </div>
        </div>
        <div class="row">
            <div class="card">
                            ...
                            <tbody>
                                @forelse($data as $value)
                                    <tr>
                                        ...
                                        <td>{{ $value->name }}</td>
                                        <td>{{ \App\Models\Category::STATUS[$value->status] }}</td>
                                        <td>
                                            <a href="{{ route('category.edit', $value->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                            <form action="{{ route('category.destroy', $value->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
