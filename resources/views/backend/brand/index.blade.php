 ...
 <div class="container">
    <div class="page-inner">
        <div
            ...
            @include('backend.layouts.partials.breadcrumb',['page_title'=>'List Of Brand'])
            <div class="ms-md-auto py-2 py-md-0">
                <a href="{{route('brand.create')}}" class="btn btn-primary  btn-round"><i class="fa fa-plus"></i> Create Brand</a>
            </div>
        </div>
        <div class="row">
            <div class="card">
                            ...
                            <tbody>
                               @forelse($data as $value)
                                    <tr>
                                        <td>{{ $value->sl }}</td>
                                        <td>{{ $value->name }}</td>
                                        <td>{{ \App\Models\Brand::STATUS[$value->status] }}</td>
                                        <td>
                                            <a href="{{ route('brand.edit', $value->id) }}" class="btn btn-sm btn-primary">Edit</a>
                                            <form action="{{ route('brand.destroy', $value->id) }}" method="POST" style="display:inline-block;">
                                                @csrf
                                                @method('DELETE')
